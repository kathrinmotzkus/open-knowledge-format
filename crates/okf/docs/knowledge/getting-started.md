...
On Linux the packaged frontend is installed in `~/docs-browser`. After an
upgrade, run `okf-http --install-browser` again. It protects locally modified
packaged files; use `--force` only when those files should be replaced.

## 2. Configure Document Roots

Browser-managed roots are stored in `~/.config/okf/config.toml` on Linux, or
under `$XDG_CONFIG_HOME/okf/config.toml` when XDG configuration is redirected.
The browser API creates this owner-private file; it never writes `.env`.
